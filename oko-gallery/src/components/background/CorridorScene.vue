...
const materialRefs: ShallowReactive<Record<number, {
...
watch(() => artworkStore.cameraPos, (newVal) => {
...
watch(() => cameraMovementComplete.value, async (newVal) => {
  ...
  if (groundMaterial.value!.displacementScale > 0) {
    gsap.to(groundMaterial.value!, {
      ...
      onComplete: () => {
        ...
        sharedHeightMap = new THREE.DataTexture(sharedHeightData, img.width, img.height, THREE.RedFormat) //TODO look into compression and sample
        ...
        for (let i = 0; i < imageData.data.length; i += 4) {
          sharedHeightData[i / 4] = (imageData.data[i] + imageData.data[i + 1] + imageData.data[i + 2]) / 3
        }
...
<template>
  <TresHemisphereLight :intensity="ambientLight.intensity" :color="ambientLight.color"
  ...
  >
    ...
    <TresMesh
      ...
      @click="onClick"
      @pointer-enter="(e) => {onPointerEnter(e);}"
      @pointer-out="(e) =>{ onPointerOut(e);}"
      :ref="(el: any)  => { if (el) { frameRefs[i] = el; } }"
      cast-shadow
      receive-shadow
      wrapAround
    >
      ...
      <TresMeshStandardMaterial
        :ref="(el: any)  => { if (el) { materialRefs[i].frame = el; } }"
        color="#151515" :metalness="0.5" :roughness="0.5"
        transparent />
      <!-- Border -->
      <TresMesh :scale="[0.9, 0.93, 0.9]" :position="[0, 0, 0.2]">
        <TresBoxGeometry />
        <TresMeshStandardMaterial
          :ref="(el: any) => { if (el) { materialRefs[i].border = el } }"
          transparent
        />
      </TresMesh>
      ...
      >
        ...
        <TresMeshPhysicalMaterial
          :ref="(el: any)  => { if (el) { materialRefs[i].image = el; } }"
          :map="textures[i]"
          :metalness="0.05"
          :roughness="0.9"
          :sheen="0.1"
  ...
  <TresMesh :rotation="[-Math.PI / 2, 0, 0]"
            ...
            ref="groundMesh"
  >
    <TresPlaneGeometry :args="[100, 100, 256, 256]" /> <!-- use enough segments for displacement -->
    <TresMeshPhysicalMaterial
      ref="groundMaterial"
      :metalness="0.5"
      :roughness="0.33"
