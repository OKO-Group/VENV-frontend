...
export async function useSunLighting(sky: any, light: DirectionalLight, ambientLight: Reactive<any>) {
  ...
  const updateSun = async () => {
    try {
      ...
      const t = Math.min(1, Math.max(0, (elevationNormalized - sunriseMin) / (sunriseMax - sunriseMin)));
