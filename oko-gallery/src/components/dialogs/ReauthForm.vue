...
const requiredRule = (v: string) => !!v || `${mode.value === MFAMethod.PASSWORD ? 'Password' : 'Code'} is required`
</script>
...
<template>
  <div>
    ...
    />
    <v-otp-input v-else v-model="inputValue" :length="6" outlined required autofocus
                 :rules="[requiredRule]" :error="'code' in authStore.errors">
    </v-otp-input>
    <v-btn
      v-if="authStore.hasTOTP"
      variant="text"
      size="small"
      class="mt-2"
      @click="toggleMode"
